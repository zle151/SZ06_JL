...
void OLED_WR_Byte(u8 dat, u8 mode) {
  if (mode) {
    // 数据
    OLED_I2C_WR(DEV_ADDR, 0x40, &dat, 1);
  } else {
    // 命令
    OLED_I2C_WR(DEV_ADDR, 0x00, &dat, 1);
  }
}
// ------------------------------------------------- OLED业务驱动
...
void OLED_Refresh(void) {
  ...
  for (i = 0; i < 8; i++)  // 外循环遍历行
  {
    ...
    OLED_I2C_WR(DEV_ADDR, 0x40, &OLED_GRAM[i][0], 128);
    //    OLED_I2C_WR2(DEV_ADDR, 0x40, OLED_GRAM[i], 8, 128);
  }
}
// 清屏函数
void OLED_Clear(u8 is_refresh) {
  ...
  if (is_refresh) OLED_Refresh();  // 更新显示
}
...
void OLED_DrawPoint(u8 x, u8 y, u8 t) {
  ...
  if (t) {  // 把指定行列的oled灯 点亮, 设置1
    OLED_GRAM[i][x] |= n;
            ...
            //		OLED_GRAM[i][x]=~OLED_GRAM[i][x];   // 0011 0010
            //		OLED_GRAM[i][x]|=n;                 // 0011 0110
            //		OLED_GRAM[i][x]=~OLED_GRAM[i][x];   // 1100 1001
